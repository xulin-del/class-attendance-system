var QRCode = (function () {
  var QR8ccBuffer = function (n) {
    this.buffer = []
    this.length = n
  }

  QR8ccBuffer.prototype = {
    get: function (index) {
  ...
  var QRUtil = {
    ...
    getLengthInBits: function (typeNumber, mode) {
      if (1 <= typeNumber && typeNumber < 10) {
        switch (mode) { case 1: return 10; case 2: return 9; case 4: return 8; case 8: return 8; default: throw 'mode:' + mode }
      } else if (typeNumber < 27) {
        switch (mode) { case 1: return 12; case 2: return 11; case 4: return 16; case 8: return 10; default: throw 'mode:' + mode }
      } else if (typeNumber < 41) {
        switch (mode) { case 1: return 14; case 2: return 13; case 4: return 16; case 8: return 12; default: throw 'mode:' + mode }
      } else throw 'type:' + typeNumber
    },
  ...
  var QRRSBlock = {
    getRSBlocks: function (typeNumber, errorCorrectionLevel) {
      ...
      if (rsBlock === undefined) throw 'bad rs block @ typeNumber:' + typeNumber + '/errorCorrectionLevel:' + errorCorrectionLevel
  ...
  QRCodeModel.createData = function (typeNumber, errorCorrectionLevel, dataList) {
    ...
    for (var i = 0; i < dataList.length; i++) {
      ...
      buffer.put(data.getLength(), QRUtil.getLengthInBits(typeNumber, data.mode))
      data.write(buffer)
    }
  ...
  QR8bitByte.prototype = {
    ...
    write: function (buffer) { for (var i = 0; i < this.data.length; i++) { var c = this.data.charCodeAt(i); if (c < 0x80) { buffer.put(c, 8) } else if (c < 0x800) { buffer.put(0xC0 | (c >> 6), 8); buffer.put(0x80 | (c & 0x3F), 8) } else { buffer.put(0xE0 | (c >> 12), 8); buffer.put(0x80 | ((c >> 6) & 0x3F), 8); buffer.put(0x80 | (c & 0x3F), 8) } } }
  }
  ...
  var drawCanvas = function (canvasId, text, options) {
    options = options || {}
    var typeNumber = options.typeNumber || 4
    var errorCorrectionLevel = options.errorCorrectionLevel || 0
    ...
    var qr = createQRCode(text, typeNumber, errorCorrectionLevel)
    var moduleCount = qr.getModuleCount()
    var cellSize = (size - padding * 2) / moduleCount
      ...
      setTimeout(function () {
        wx.canvasToTempFilePath({
          ...
          success: function (res) {
            if (options.callback) options.callback(res.tempFilePath)
          },
          fail: function (err) {
  ...
  return { drawCanvas: drawCanvas, createQRCode: createQRCode }
})()
